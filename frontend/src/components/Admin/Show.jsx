...
const handleComment = (postId, commentText) => {
  axios
    .post(`http://localhost:5000/Post/${postId}/comment`, { text: commentText })
    .then((response) => {
...
const handleDeleteComment = (postId, commentIndex) => {
  axios
    .delete(`http://localhost:5000/Post/${postId}/comment/${commentIndex}`)
    .then(() => {
      // Comment deleted successfully, update the local state
      const updatedPosts = [...posts];
      const postToUpdate = updatedPosts.find((post) => post._id === postId);
  ...
  <div className="p-4">
      ...
      {loading ? (
        <div className="text-center">Loading...</div>
         ...
         <div className="flex flex-col justify-center items-center m-auto gap-5 w-full lg:w-2/4">
          {Array.isArray(posts) &&
            posts.map((post) => (
            <div key={post._id} className="p-5 border-2 border-gray-100" style={{ width: "100%" }}>
...
>
  ...
  {commentInputs[post._id] && (
    <span className="ml-1 hidden">{post.comments.length}</span>
  )}
...
{commentInputs[post._id] && (
  <div className="post w-full flex justify-between p-5 gap-5">
    ...
    />
    <button className='bg-blue-500 p-5 rounded text-white font-semibold hover:bg-blue-700' onClick={() => handlePostComment(post._id)}>Post</button>
  </div>
)}
    ...
    {post.comments.length > 0 && (
      <div className="text-gray-900 font-poppins w-full bg-slate-200 p-5">
        ...
        {post.comments.map((comment, index) => (
         <div key={index} className="comment flex justify-between p-2 rounded text-start items-start w-ful">
  <p className="text-start bg-white w-full p-2 text-[12px] flex justify-between overflow-scroll h-[50px]" >{comment.text}
  <span className='text-[10px] text-gray-400'>
    {format(new Date(post.createdAt), "MMMM d, yyyy")}
  </span>
   <button className="bg-blue-500 p-2 text-[12px] text-white font-semibold hover:bg-blue-700" onClick={() => handleDeleteComment(post._id, index)}>
    Delete
  </button>
  </p>
                ...
                <div className="flex justify-around items-center bg-slate-900 p-5 mt-5 font-poppins">
                  <Link
                    to={`/edit/${post._id}`}
